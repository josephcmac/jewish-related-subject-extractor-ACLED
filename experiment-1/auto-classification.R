...
df1 <- read.csv("../../../datasets/acleddata/experiment-1/sample.csv") %>%
    	select(event_id_cnty, actor1, actor2,assoc_actor_1, assoc_actor_2, notes) %>%
    	classify_data %>% select(event_id_cnty, auto)

df2 <- read.csv("../../../datasets/acleddata/experiment-1/classification-1.csv") %>%
	select(event_id_cnty, manual)

merge(df1, df2) %>%
	write.csv("../../../datasets/acleddata/experiment-1/classification-2.csv", row.names=FALSE)
